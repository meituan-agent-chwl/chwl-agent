...
API_KEY = os.environ.get("LONGCAT_API_KEY", "ak_2GI2Lc7BU1y75GE1IP9pV1M357M2S")
...
class ChatAgent:
    ...
    async def handle_message(self, user_input: str) -> str:
        ...
        if self.mock_llm:
            ...
            # 收集已知用户信息（从 conversation_history 提取）
            known_info = ""
    ...
    async def _do_plan(self, user_input: str) -> str:
        ...
        if self.session_id:
            ...
            await asyncio.sleep(0.3)
                # 把对话历史拼到 user_input 里，保证 Phase 1 的 LLM 能拿到完整上下文
        if not self.mock_llm and len(self.conversation_history) > 2:
            # 取最近 3 条对话拼成完整上下文
            ctx_msgs = [m['content'] for m in self.conversation_history[-5:-1] if m.get('role') == 'user']
            enriched = ' | '.join(ctx_msgs + [user_input])
        else:
            enriched = user_input
